...
class Jaeger implements Tracer{
     ...
     * @return Span
     */
    public function startSpan($operationName, $options = []): \OpenTracing\Span{
    ...
    public function startActiveSpan($operationName, $options = []): \OpenTracing\Scope{
        if (!$options instanceof StartSpanOptions) {
            $options = StartSpanOptions::create($options);
        }
    ...
    private function getParentSpanContext(StartSpanOptions $options)
    {
        ...
        if ($parentSpan) {
            if (($parentSpan->isValid()
                || (!$parentSpan->isTraceIdValid() && $parentSpan->getDebugId())
                || count($parentSpan->getBaggage()) > 0)
            ) {
                return $parentSpan;
            }
        }
